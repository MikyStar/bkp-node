...
export namespace Archive {
  export const compress = async ({ sourcePath, destPath }: Args) => {
    try {
      if (System.isDirectory(sourcePath)) await compressing[ARCHIVE_ALGO].compressDir(sourcePath, destPath);
      else await compressing[ARCHIVE_ALGO].compressFile(sourcePath, destPath);
    } catch (e) {
      console.log(e);
