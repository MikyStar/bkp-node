...
export class ArgHandler {
  ...
  private setAction = (arg: string) => {
    ...
    if (allowedCreate.includes(arg)) this.action = Action.CREATE;
    else if (allowedExtract.includes(arg)) this.action = Action.EXTRACT;
    else throw new FirstArgError(arg);
  };

  private setSource = (arg: string) => {
    if (!System.doesFileExists(arg)) throw new FileNotFoundError(arg);
  ...
  private setDest = (arg: string) => {
    ...
    if (this.action === Action.CREATE) path = arg || this.sourcePath + DEFAULT_CREATE_EXTENSION;
    else {
      if (arg) path = arg;
      else {
        const regex = new RegExp(`${DEFAULT_CREATE_EXTENSION}$`);

        if (regex.test(this.sourcePath)) path = this.sourcePath.split(DEFAULT_CREATE_EXTENSION)[0];

        path = this.sourcePath + DEFAULT_EXTRACT_EXTENSION;
      }
    }

    if (System.doesFileExists(path)) throw new FileAlreadyExistsError(path);
  ...
  private extractOccurances = (callback: (arg: string, index?: number) => boolean): ArgOccurance[] | undefined => {
    ...
    for (let i = occurances.length - 1; i >= 0; i--) this.args.splice(occurances[i].index, 1);
